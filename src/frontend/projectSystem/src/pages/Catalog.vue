...
import { api, type Offering, type SubmitError } from '../api';
...
const tab = ref('all');
const search = ref('');
const banner = ref<{ type: 'ok' | 'error'; text: string; details?: string[] } | null>(null);
...
function formatStudentError(e: SubmitError): string {
  const msg = (e.message || '').trim();
  if (e.code === 'prerequisite') return msg.includes('先修') ? msg : `先修课未满足：${msg}`;
  if (e.code === 'full') return msg.includes('名额') ? msg : `课程名额已满：${msg}`;
  if (e.code === 'conflict') {
    if (msg.includes('时间冲突')) return msg;
    if (msg.includes('关闭') || msg.includes('取消')) return msg;
    if (msg.includes('超过') || msg.includes('重复') || msg.includes('缺少')) return msg;
    return msg ? `课表冲突：${msg}` : '课表存在冲突';
  }
  return msg;
}

function showValidationFail(action: '提交注册' | '保存课表', message?: string, errors: SubmitError[] = []) {
  const details = [...new Set(errors.map(formatStudentError).filter(Boolean))];
  const rowMap: Record<string, string> = {};
  errors.forEach(e => { if (e.offeringId) rowMap[e.offeringId] = e.message; });
  errorRows.value = rowMap;
  banner.value = {
    type: 'error',
    text: details.length ? `无法${action}，请按下列原因修正后再试：` : (message || `${action}失败，请稍后再试。`),
    details,
  };
}
...
async function save() {
  ...
  try {
    ...
    showValidationFail('保存课表', e?.message, e?.errors ?? []);
  }
}
...
async function submit() {
  ...
  if (r.ok) {
    ...
    showValidationFail('提交注册', r.message, r.errors);
  }
}
...
<template>
  <div class="content">
    ...
    <div v-if="banner" class="banner" :class="banner.type">
      <p>{{ banner.text }}</p>
      <ul v-if="banner.details?.length">
        <li v-for="(d, i) in banner.details" :key="i">{{ d }}</li>
      </ul>
    </div>
...
.banner p { margin: 0; }
.banner ul { margin: 8px 0 0; padding-left: 1.2em; font-weight: 600; line-height: 1.6; }
