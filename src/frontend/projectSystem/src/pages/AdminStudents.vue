...
function requestDelete(r: StudentRow) { pendingDelete.value = r; }
...
async function confirmDelete() {
  ...
  error.value = '';
  banner.value = '';
  try { await api.deleteStudent(r.id); banner.value = `学生 ${r.name} 已删除。`; await load(); }
  catch (e: any) { error.value = e.message || '删除失败'; }
}

const blank = () => ({ name: '', dob: '', ssn: '', status: '在读', graduationDate: '' });
const form = reactive(blank());

async function load() { rows.value = await api.students(); }

onMounted(async () => { await load(); loading.value = false; });

function edit(r: StudentRow) {
  editingId.value = r.id;
  Object.assign(form, { name: r.name, dob: r.dob, ssn: r.ssn, status: r.status, graduationDate: r.graduationDate });
}

function cancelEdit() { editingId.value = null; Object.assign(form, blank()); }

async function submit() {
  if (!form.name || !form.dob || !form.ssn) { error.value = '请填写姓名、出生日期和证件号。'; return; }
  busy.value = true;
  error.value = '';
  banner.value = '';
  try {
    if (editingId.value) {
      ...
      banner.value = '学生信息已更新。';
    } else {
      const created = await api.addStudent({ ...form });
      banner.value = `学生已添加，学号：${created.id}。`;
    }
    cancelEdit();
    await load();
...
<template>
  <div class="content">
    ...
    <section class="form-card">
      <span class="eyebrow">{{ editingId ? `编辑学生（${editingId}）` : '新增学生' }}</span>
      <form class="form-grid" @submit.prevent="submit">
        <label>姓名<input v-model="form.name" placeholder="学生姓名" /></label>
        <label>出生日期<input v-model="form.dob" type="date" /></label>
        <label>证件号<input v-model="form.ssn" placeholder="身份证号" /></label>
        <label>状态<select v-model="form.status"><option>在读</option><option>休学</option><option>毕业</option></select></label>
        <label>毕业日期<input v-model="form.graduationDate" type="date" /></label>
        <div class="form-actions">
          <button class="primary" :disabled="busy">{{ busy ? '提交中…' : editingId ? '保存修改' : '添加学生' }}</button>
    ...
    <div class="table-card">
      <form class="search-bar" @submit.prevent="doSearch">
        <input v-model="keyword" placeholder="输入学号或姓名检索，如 S001" />
      ...
      <p v-if="loading" class="muted">学生列表加载中…</p>
      <table v-else>
        <thead><tr><th>学号</th><th>姓名</th><th>出生日期</th><th>证件号</th><th>状态</th><th>毕业日期</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="r in filteredRows" :key="r.id">
            <td>{{ r.id }}</td><td>{{ r.name }}</td><td>{{ r.dob }}</td><td>{{ r.ssn }}</td>
            <td><span class="pill" :class="r.status === '在读' ? 'st-open' : 'st-full'">{{ r.status }}</span></td>
            <td>{{ r.graduationDate }}</td>
            <td class="row-actions">
              <button @click="edit(r)">编辑</button>
              <button class="danger" @click="requestDelete(r)">删除</button>
            </td>
          </tr>
          <tr v-if="notFound"><td colspan="7" class="muted not-found">未找到学号或姓名为「{{ query }}」的学生，请重新输入或清除检索。</td></tr>
          <tr v-else-if="!filteredRows.length"><td colspan="7" class="muted">暂无学生信息。</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <transition name="modal-fade">
      <div v-if="pendingDelete" class="modal-mask" @click.self="cancelDelete">
        <div class="modal-box">
          <h4>确认删除学生？</h4>
          <p>将删除学生 {{ pendingDelete.name }}（{{ pendingDelete.id }}），此操作不可恢复。</p>
