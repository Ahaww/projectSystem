...
const OFFERINGS: Offering[] = [
  ...
  { id: 'CS-201', code: 'CS-201', title: '数据结构', dept: '计算机科学', professor: 'Chen 博士', days: ['周二', '周四'], start: 480, end: 570, room: '科学楼 206', seatsTotal: 10, seatsTaken: 3, prerequisites: ['CS-101'] },
  { id: 'CS-350', code: 'CS-350', title: '操作系统', dept: '计算机科学', professor: 'Morgan 博士', days: ['周五'], start: 540, end: 720, room: '科学楼 208', seatsTotal: 10, seatsTaken: 1, prerequisites: ['CS-210'] },
  { id: 'MATH-301', code: 'MATH-301', title: '线性代数', dept: '数学', professor: 'Lin 教授', days: ['周一', '周三'], start: 660, end: 750, room: '数学楼 201', seatsTotal: 10, seatsTaken: 5, prerequisites: [] },
  { id: 'ENG-201', code: 'ENG-201', title: '科技写作', dept: '人文', professor: 'Lee 博士', days: ['周四'], start: 540, end: 630, room: '东楼 108', seatsTotal: 10, seatsTaken: 3, prerequisites: [] },
  { id: 'CHEM-101', code: 'CHEM-101', title: '普通化学', dept: '自然科学', professor: 'Kim 教授', days: ['周二', '周四'], start: 780, end: 870, room: '实验楼 201', seatsTotal: 10, seatsTaken: 4, prerequisites: [] },
  { id: 'ECON-210', code: 'ECON-210', title: '微观经济学', dept: '商学', professor: 'Patel 博士', days: ['周三'], start: 660, end: 750, room: '西楼 201', seatsTotal: 10, seatsTaken: 7, prerequisites: ['BUS-101'] },
];

const COMPLETED = ['CS-101', 'MATH-110', 'ENG-105'];   // 学生已修课程（用于先修课校验）
const USERS: Record<string, { password: string; user: User }> = {
  S001: { password: 'S001', user: { id: 'S001', name: 'Jordan Davis', role: 'STUDENT' } },
  S002: { password: 'S002', user: { id: 'S002', name: '李四', role: 'STUDENT' } },
  S003: { password: 'S003', user: { id: 'S003', name: '王五', role: 'STUDENT' } },
  S004: { password: 'S004', user: { id: 'S004', name: '赵六', role: 'STUDENT' } },
  S005: { password: 'S005', user: { id: 'S005', name: '孙七', role: 'STUDENT' } },
  P001: { password: 'P001', user: { id: 'P001', name: 'Alex Morgan', role: 'PROFESSOR' } },
  P002: { password: 'P002', user: { id: 'P002', name: 'Lin 教授', role: 'PROFESSOR' } },
  P003: { password: 'P003', user: { id: 'P003', name: 'Chen 博士', role: 'PROFESSOR' } },
  P004: { password: 'P004', user: { id: 'P004', name: 'Lee 博士', role: 'PROFESSOR' } },
  P005: { password: 'P005', user: { id: 'P005', name: 'Patel 博士', role: 'PROFESSOR' } },
  P006: { password: 'P006', user: { id: 'P006', name: 'Kim 教授', role: 'PROFESSOR' } },
  P007: { password: 'P007', user: { id: 'P007', name: 'Dai 博士', role: 'PROFESSOR' } },
  R001: { password: 'R001', user: { id: 'R001', name: '王丽', role: 'REGISTRAR' } },
};
...
function failMessage(
  hint: string,
  errors: { offeringId: string; code: string; message: string }[],
) {
  const details = [...new Set(errors.map(e => e.message).filter(Boolean))].join('；');
  return details
    ? `课表校验未通过，${hint}：${details}`
    : `课表校验未通过，${hint}`;
}
...
const PROFESSORS: ProfessorRow[] = [
  ...
  { id: 'P005', name: 'Patel 博士', dob: '1978-06-20', ssn: '110101197806200088', status: '在职', dept: '商学' },
  { id: 'P006', name: 'Kim 教授', dob: '1977-03-11', ssn: '110101197703110099', status: '在职', dept: '自然科学' },
  { id: 'P007', name: 'Dai 博士', dob: '1982-08-19', ssn: '110101198208190010', status: '在职', dept: '艺术' },
];
...
export const mock = {
  ...
  async saveSchedule(items: ScheduleItem[]): Promise<Schedule> {
    ...
    if (errors.length) throw new ApiError(422, failMessage('请修正后重新保存', errors), errors);
  ...
  async submitSchedule(items: ScheduleItem[]): Promise<Schedule> {
    ...
    // 所有两两时间冲突（主主 / 主备 / 备备）
    errors.push(...checkTimeConflicts(items));
    if (errors.length) throw new ApiError(422, failMessage('请修正后重新提交', errors), errors);
  ...
  async addStudent(d: Omit<StudentRow, 'id'>) {
    ...
    STUDENTS.push(row);
    USERS[row.id] = { password: row.id, user: { id: row.id, name: row.name, role: 'STUDENT' } };
    return clone(row);
  },
  ...
  async addProfessor(d: Omit<ProfessorRow, 'id'>) {
    ...
    PROFESSORS.push(row);
    USERS[row.id] = { password: row.id, user: { id: row.id, name: row.name, role: 'PROFESSOR' } };
    return clone(row);
  },
